...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id'])) {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $sqlpro = "INSERT INTO `turf_owner` (`id`, `venue_name`, `owner_name`, `email`, `phone_number`, `locationLink`, `password`) 
                   VALUES (NULL, '{$requestData['venueName']}', '{$requestData['fullName']}', '{$requestData['email']}', '{$requestData['phoneNumber']}', '{$requestData['locationLink']}', '{$uniquePassword}')";
        ...
        if ($quepro) {
            ...
            $message = "Hey, {$requestData['fullName']}, your request for the registration of {$requestData['venueName']} has been approved. Your login credentials for your admin panel are username={$requestData['email']} and password={$uniquePassword}";
